...
const Student *findStudent(const Student *studentList, const char *name) {
    int student_found = strncmp(studentList->name, name, strlen(name));
    while ((student_found != 0) && (studentList->next != NULL)) {
        studentList = studentList->next;
        student_found = strncmp(studentList->name, name, strlen(name));
    }
    if (student_found == 0) {
        return studentList;
    } else {
